...
def safehouse(self):
        if self.state == "safehouse":
            ...
            if not self.start:
                ...
                if self.prevlevel == str(self.player.level):
            ...
            if self.level == "safehouse":
                self.level_transition(200, "Welcome to the safehouse", tips=True)
            if self.store:
                ...
                self.level_transition(50, tips=False)
                img = pygame.Surface((1200, 675))
                img.fill((0,0,0))
                img.set_alpha(150)
                self.display.blit(img, (0,0))
                ...
                elif int(self.level.split("_")[1]) <= int(self.player.level):
